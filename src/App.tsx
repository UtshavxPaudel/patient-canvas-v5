      ...
      try {
        ...
        es.addEventListener('focus-item', (event: any) => {
          try {
            ...
            console.log('🎯 Focus-item event received via SSE:', data);
            handleFocusRequest({
        ...
        es.addEventListener('new-item', (event: any) => {
          try {
            ...
            // Position the item at viewport center + 500px in both X and Y
            try {
              const center = (window as any).getViewportCenterWorld?.();
              if (center) {
                const width = newItem.width || 420;
                const height = newItem.height || 300;
                const x = Math.round(center.x - width / 2 + 500); // Offset 500px to the right
                const y = Math.round(center.y - height / 2 + 500); // Offset 500px down
                newItem.x = x;
                newItem.y = y;
                
                // Persist position to backend
                fetch(`${API_BASE_URL}/api/board-items/${newItem.id}`, {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
            ...
            // Add the new item to the frontend state
            setItems((prev: any[]) => {
